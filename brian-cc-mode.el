;; use experimental cc-mode for C++11 feature support

;; (add-to-list 'load-path "~/code/cc-mode-beta")
...
;; Pulled this from stackoverflow - TODO prolly delete i think it sux!
...
(defun brian-c-lineup-template-args (langelem)
  ...
  (let* ((ob-pos (brian-c-lineup-template--calc-open-bracket-pos langelem))
	 ...
	 (up-list-offset (save-excursion
			   (backward-up-list)
			   (point)))
	 (offset (brian-c-lineup-template--calc-indent-offset
		  (max up-list-offset ob-pos))))
...
(c-add-style 
 "briancpp" '((c-basic-offset . 2)
	      ...
	      (c-cleanup-list . (brace-else-brace
	       ...
	       . (
		  (defun-open . brian-c-hanging-defun-open)
		  ;; (defun-close . brian-c-hanging-defun-close)
	      ...
	      (c-offsets-alist
	       ...
	       (defun-close . 0)
	       (inextern-lang . 0)
	       (block-close . brian-c-block-close-offset)
...
(defun brian-c-statement-cont (langelem)
  (save-excursion
    ;; trying to make brace init'd constructors w/lambdas not suck (workaround)
    (if (assq 'defun-block-intro c-syntactic-context)
	0
      '+)))

(defun brian-c-block-close-hanging-brace (syntax pos)
  (save-excursion
    (when (c-beginning-of-syntax)
      ;;(pp (assq 'member-init-cont c-syntactic-context))
      '(after))))
...
(defun brian-c-hanging-defun-open (syntax pos)
  (save-excursion
    ;; (message (format "HAHA!: %s" syntax))
    (let (langelem)
      (if (and (eq syntax 'defun-open)
	       (setq langelem (assq 'defun-open c-syntactic-context)))
	  (progn (goto-char (c-langelem-pos langelem))
		 ;; (pp (list (point) langelem))
		 (if (assq 'statement (c-guess-basic-syntax))
		     nil
		   '(before after)))
	'(before after)))))

;; (defun brian-c-hanging-defun-close (syntax pos)
;;   (save-excursion
;;     (message (format "HOHO!: %s" syntax))
;;     (let (langelem)
